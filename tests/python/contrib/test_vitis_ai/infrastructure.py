...
"""Expose Vitis-AI test functions to the Python front end"""
import sys
import numpy as np
...
def verify_codegen(module, num_vitis_ai_modules=1, params=None,
    ...
    module, lib, params = build_module(module, target, params=params, dpu_target=dpu_target)
    vitis_ai_modules = extract_vitis_ai_modules(lib)
...
def verify_result(mod, map_inputs, out_shape, result, tol=1e-5, target="llvm",
    ...
    graph, lib, params = build_module(mod, target, params=params,
                                      dpu_target=dpu_target, tvm_ops=tvm_ops)
    lib = update_lib(lib)
    ctx = tvm.cpu()
    rt_mod = tvm.contrib.graph_runtime.create(graph, lib, ctx)
